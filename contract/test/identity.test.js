...
contract(Identity,async(accounts)=>{
    ...
    describe('Identity Contract',async()=>{
        ...
        it('Register 1 Issuer',async()=>{
            ...
            await identity.registerUser("My Public Key" , {from : accounts[0]});
            await identity.registerUser("My Public key [1]", {from : accounts[1]});
            await identity.requestIssuerAccount("I want to issue" , "National Id",{from : accounts[1]});
            await identity.verifyIssuerAccount(0,{from : accounts[0]});
        ...
        it('Add 1 id',async()=>{
            await identity.registerUser("My Public Key" , {from : accounts[0]});
            await identity.registerUser("My Public key [1]", {from : accounts[1]});
            await identity.requestIssuerAccount("I wannt to issue" , "National Id",{from : accounts[1]});
            await identity.verifyIssuerAccount(0,{from : accounts[0]});
